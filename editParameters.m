...
for paramIndex = 1:paramCount
    ...
    if isinteger(defaultValue)
        ...
        uicontrol(...
            ...
            'CData', push_cdata, ...
            'Callback', @(hObject,eventdata)stepValue(hObject, eventdata, guidata(hObject), paramTag, 'up'));
        uicontrol(...
            'Parent', handles.figure,...
            'Units', 'points', ...
            'Position', [labelWidth+201 dialogHeight-(paramIndex+1)*30-1 12 12], ...
            'CData', flipdim(push_cdata, 1), ...
            'Callback', @(hObject,eventdata)stepValue(hObject, eventdata, guidata(hObject), paramTag, 'down'));
    ...
    elseif iscell(defaultValue)
        ...
        handles.(paramTag) = uicontrol(...
            ...
            'Callback', @(hObject,eventdata)popUpMenuChanged(hObject, eventdata, guidata(hObject), paramTag), ...
            'String', defaultValue, ...
            'Style', 'popupmenu', ...
            'Value', popupValue, ...
            'Tag', paramTag);
...
for i = 1:length(textFieldParamNames)
    ...
    try
        ...
        set(javaHandle, 'KeyTypedCallback', {@valueChanged, hObject});
    catch ME %#ok<NASGU>
    end
end
...
for paramIndex = 1:paramCount
    ...
    try
        ...
        setParamValueInUI(handles, paramName, paramValue, temp{paramChannel});
    end
end
handles.edited = true;
guidata(handles.figure, handles);
...
function popUpMenuChanged(hObject, eventdata, handles, paramTag)
if strcmp(paramTag, 'CHANNELSEdit')
    handles.protocolCopy.selectedChannel = get(handles.(paramTag), 'value');
    useDefaultParameters(hObject, eventdata, handles);
end
end


function valueChanged(~, ~, hObject)
...
function stepValue(~, ~, handles, paramTag, direction)
...
for paramIndex = 1:paramCount
    ...
    temp = handles.protocolCopy.protocolProperties(paramName);
    if iscell(defaultValue)
        if ~isempty(temp{channels + 1}{paramChannel})
            dV = temp{channels + 1}{paramChannel};
        elseif strcmp('CHANNELS', paramName)
            dV = paramChannel;
        else
            dV = 1;
        end
...
paramNames = keys(params);
paramCount = params.Count;
paramChannel = handles.protocol.selectedChannel;

paramChannels = handles.protocol.channels;
...
for paramIndex = 1:paramCount
    ...
    defaultValue = paramProps{paramChannel};
        ...
        temp = handles.protocolCopy.protocolProperties(paramName);
        if iscell(defaultValue)
            temp{paramChannels+1}{paramChannel} = paramValue;
        else
            temp{paramChannel} = paramValue;
        end
        handles.protocolCopy.protocolProperties(paramName) = temp;
